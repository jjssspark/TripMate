...
export const handler = async (event: any) => {
  ...
  // server/server.ts와 같은 값을 유지해야 로컬과 배포 결과가 갈리지 않는다.
  ...
  console.log(`- 여행 강도: ${intensity || "여유롭게"} (하루 ${spotRange.min}~${spotRange.max}스팟 + 식사 3개)`);
...
반드시 명시된 JSON 스키마를 준수하여 응답해 주세요.`;
  ...
  // server/server.ts와 같은 값을 유지해야 로컬과 배포 동작이 갈리지 않는다.
  ...
  try {
    ...
    try {
      ...
      response = await withTimeout(generateWithModel(ai, "gemini-2.5-flash"), remainingMs() - 3_000, "주 모델");
      ...
      try {
        response = await withTimeout(generateWithModel(ai, "gemini-2.5-flash-lite"), remainingMs() - 2_000, "보조 모델");
        ...
        await sleep(1000);
        response = await withTimeout(generateWithModel(ai, "gemini-2.5-flash"), remainingMs() - 1_000, "주 모델 최종 시도");
    ...
    try {
      ...
      console.warn("★ [Netlify Function] 실사진 조회가 예산을 넘겨 목업 이미지로 대체합니다:", photoErr);
